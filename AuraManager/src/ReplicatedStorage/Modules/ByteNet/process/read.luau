...
return function(incomingBuffer: buffer, references: { [number]: unknown }?, player: Player?)
	...
	while readCursor < length do
		...
		--if not packet then
		--	--repeat task.wait() packet = ref[buffer.readu8(incomingBuffer, readCursor)] until packet
		--	print("packet not found :",buffer.readu8(incomingBuffer, readCursor))
		--end

		if not packet then print(`packet: `, {packet}) end
